...
module.exports = function loadPlayer(id, player) {
    ...
    anvp[id] = {
        config: {
            accessKey: player.getAttribute('data-access-key'),
            video: player.getAttribute('data-videoid'),
                ...
                plugins: {
                    dfp: {
                        clientSide: buildAdConfig(
                            scrubMetadata(video),
                            id,
                            player.getAttribute('data-cmsid'),
                            parseInt(player.getAttribute('data-dfp-timeout')),
                            // Only the first video played uses the VPX's topics and categories.
                            playCount === 1 ? player.getAttribute('data-topics') : [],
                            playCount === 1 ? player.getAttribute('data-categories') : []
                        )
                    }
                }
            };
